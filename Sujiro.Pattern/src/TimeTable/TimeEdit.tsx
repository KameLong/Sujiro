...
export default function TimeEdit({time,onChange,deleteTime,closeTimeEdit}:TimeEditProps) {
    ...
    return(
        ...
        <Grid container spacing={2} style={{margin:'0px auto',width:'360px'}}>
            ...
            <Grid xs={3} >
                <Button variant={"contained"} style={{backgroundColor:'lightpink',color:'black',padding:'5px'}}
                onClick={()=>{
                    deleteTime();
                }}>時刻削除</Button>
            </Grid>
            ...
            <Grid xs={2} p={0.5}>
                <Button className={style.timeButton} variant={'contained'} style={{backgroundColor:'lightBlue',color:'black'}}
            ...
            <Grid xs={4} >
                <FormControlLabel style={{fontSize:'10px'}} control={<Checkbox size={"small"} value={timeShiftPattern.retroactive}
                     onChange={(e)=>{timeShiftPattern.setRetroactive(e.target.checked)}}/>
                } label="前倒し"/>
            </Grid>
            <Grid xs={2} p={0.5}>
                <Button className={style.timeButton} variant={'contained'} style={{backgroundColor:'lightGreen',color:'black'}}
                        onClick={()=>timeShift(-10)}>
                -10秒
                </Button>
            </Grid>
